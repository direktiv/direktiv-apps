...
// Defaults
const (
	code               = "com.gcloud-instance-delete.error"
	GCP_AuthURL        = "https://www.googleapis.com/auth/compute"
	GCP_RequestTimeout = 80 * time.Second
)

func main() {
	direktivapps.StartServer(GCPComputeDelete)
}

func GCPComputeDelete(w http.ResponseWriter, r *http.Request) {
	...
	// Do Request
	requestURL := fmt.Sprintf("https://www.googleapis.com/compute/v1/projects/%s/zones/%s/instances/%s", obj.Project, obj.Zone, obj.InstanceID)
